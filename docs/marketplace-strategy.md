...
Add a build step that emits host-specific artifacts into `dist/` for CI and
release packaging:
...
For hosts that require marketplace files at a repository root, publish those
generated artifacts into a dedicated marketplace repository, release branch, or
checked-in generated directory with clear ownership. Do not make humans edit
...
For `/plugin marketplace add LVTD-LLC/skills` to work directly, the published
repository root must contain `.claude-plugin/marketplace.json`. Otherwise, use a
dedicated marketplace repository or branch that contains the generated Claude
marketplace root.
...
- Generate Claude Code marketplace artifacts.
- Generate Codex marketplace artifacts.
- Generate OpenClaw publish plan.
- Keep generated files out of source control unless a host requires them at a
  repository root for Git-based installation.
