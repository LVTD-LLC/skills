# Triage Doc Feedback Workflow

Turn documentation feedback into prioritized action.

## When To Use

- Reviewing page feedback, support trends, survey results, or docs issues.
- Setting up a feedback intake template.
- Deciding what documentation fixes to prioritize.

## Prerequisites

- Feedback item or feedback set.
- Affected doc URL, product context, or support evidence when available.

**Reference**: from this workflow file, open `../references/core/knowledge.md`.

## Workflow Steps

### Step 1: Normalize The Feedback

**Goal**: Preserve enough context to act.

- [ ] Capture source, URL, section, user goal, and product/version.
- [ ] Summarize the problem in one sentence.
- [ ] Preserve exact error messages or search terms when available.
- [ ] Identify missing context to request.

### Step 2: Validate And Route

**Goal**: Decide whether this is docs work.

- [ ] Check whether the issue is current and reproducible.
- [ ] Search for duplicates.
- [ ] Classify as docs, product, support, policy, or unclear.
- [ ] Route non-doc issues while preserving docs implications.

### Step 3: Assess Actionability

**Goal**: Determine whether a fix can be made now.

- [ ] Identify what should change: page, section, sample, link, prerequisite, concept, or navigation.
- [ ] Identify owner and source of truth.
- [ ] Decide whether more user detail or technical review is needed.

### Step 4: Prioritize

**Goal**: Put work in impact order.

- [ ] Estimate severity and number of affected users.
- [ ] Check whether users are blocked or harmed.
- [ ] Apply P0-P3 priority.
- [ ] Note supporting evidence such as support volume or analytics.

### Step 5: Close The Loop

**Goal**: Keep feedback channels trustworthy.

- [ ] Create or update the work item.
- [ ] Reply to reporter when contact exists and the report is useful.
- [ ] Record the resolution or reason for no action.
- [ ] Watch for repeat feedback after the fix.

## Exit Criteria

- [ ] Feedback is classified, prioritized, and routed.
- [ ] Actionable docs work has an owner and next step.
- [ ] Unactionable or non-doc feedback is not left ambiguous.
